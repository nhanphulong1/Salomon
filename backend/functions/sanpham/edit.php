    ...
    <div class="container-fulid">
        <div class="row">
            ...
            <?php
                ...
                if(!isset($_GET['sp_ma'])) echo "<script>location.href = 'index.php';</script>";
                $sp_ma = $_GET['sp_ma'];
                $query_km="select * from sanpham where sp_ma='$sp_ma'";

                $result_sanpham=mysqli_query($conn,$query_km);
                $sanpham = mysqli_fetch_array($result_sanpham,MYSQLI_ASSOC);
            ...
            <div class="col-md-9 content">
                ...
                <form action="" method="post" name="frmSanPham" id="frmSanPham">
                    ...
                    <input type="text" name="sp_ten" class="form-control" value="<?=$sanpham['sp_ten'] ?>"><br>
                    <label>Nhập giá SP:</label><br>
                    <input type="number" name="sp_gia" min="0" value="0" class="form-control" value="<?=$sanpham['sp_gia'] ?>"><br>
                    <label>Nhập mô tả ngắn:</label><br>
                    <input type="text" name="sp_mota_ngan" class="form-control" value="<?=$sanpham['sp_mota_ngan'] ?>"><br>
                    <label>Nhập mô tả chi tiết:</label><br>
                    <textarea name="sp_mota_chitiet" class="form-control" rows="6"><?=$sanpham['sp_mota_chitiet'] ?></textarea><br>
                    <label>Nhập số lượng SP:</label><br>
                    <input type="number" name="sp_soluong" min="0" value="0" class="form-control" value="<?=$sanpham['sp_soluong'] ?>"><br>
                    <label>Nhập loại sản phẩm:</label><br>
                    <select name='lsp_ma' class="form-control">
                        <?php foreach($result_lsp as $lsp): ?>
                        <option value="<?= $lsp['lsp_ma'] ?>"  <?= ($lsp['lsp_ma']==$sanpham['lsp_ma'] ? 'selected':'') ?>  ><?= $lsp['lsp_ten'] ?></option>
                        <?php endforeach ?>
                    </select>
                    <br>
                    <select name='nsx_ma' class="form-control">
                        <?php foreach($result_nsx as $nsx): ?>
                        <option value="<?= $nsx['nsx_ma'] ?>" <?= ($nsx['nsx_ma']==$sanpham['nsx_ma'] ? 'selected':'') ?> ><?= $nsx['nsx_ten'] ?></option>
                        <?php endforeach ?>
                    </select>
                    <br>
                    <select name='km_ma' class="form-control">
                        ...
                        <option value="<?= $km['km_ma'] ?>" <?= ($km['km_ma']==$sanpham['km_ma'] ? 'selected':'') ?> ><?= $km['km_noidung'] ?></option>
                        <?php endforeach ?>
                    </select>
                    <br>
